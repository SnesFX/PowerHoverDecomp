...
namespace FluffyUnderware.Curvy.Examples
{
	public class Performance : MonoBehaviour
	{
		...
		private void OnGUI()
		{
			...
			splineSelection = GUILayout.SelectionGrid(splineSelection, new string[4] { "Linear", "Catmul-Rom", "TCB", "Bezier" }, 4);
			...
			GUI.enabled = !running;
			if (GUILayout.Button((!running) ? "Run!" : "Please wait..."))
			{
		...
		private CurvySpline CreateSpline(CurvyInterpolation type, int points, int granularity)
		{
			...
			{
				curvySpline.Add(null, false).Position = UnityEngine.Random.insideUnitCircle * 10f;
			}
		...
		{
			string text = Application.dataPath + "/CurvyPerformance_" + string.Format("{0:yyyyMMdd_HHmm}", DateTime.Now) + ".csv";
			...
			foreach (Result result in Results)
			{
				...
				{
					double num = value;
					stringBuilder.Append(string.Format(";{0:0.0000}", num));
				}
				stringBuilder.AppendLine(";");
			}
			File.WriteAllText(text, stringBuilder.ToString());
			Debug.Log("File saved as '" + text + "'");
		}
	}
}
