...
public class NotificationExample : BaseIOSFeaturePreview
{
	...
	private void Awake()
	{
		...
		{
			IOSMessage.Create("Launch Notification", string.Concat("Messgae: ", launchNotification.Content, "\nNotification ID: ", launchNotification.Id));
		}
	...
	public void CheckNotificationSettings()
	{
		...
		if (((uint)allowedNotificationsType & 2u) != 0)
		{
			Debug.Log("Sound avaliable");
		}
		if (((uint)allowedNotificationsType & (true ? 1u : 0u)) != 0)
		{
			Debug.Log("Badge avaliable");
		}
		if (((uint)allowedNotificationsType & 4u) != 0)
		{
